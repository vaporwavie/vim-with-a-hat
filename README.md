...
goes away. That's the whole idea — a "quick open" you can fire at a file and
dismiss, without launching a full terminal app.
...
`vh` boots a minimal `NSApplication`, puts a `SwiftTerm` terminal view in a
single window, and spawns your editor in a PTY with the file paths as arguments.
It inherits your environment and working directory (so relative paths and your
editor config just work), forces `TERM=xterm-256color` + `COLORTERM=truecolor`,
mirrors the window title from the editor, and exits with the editor's own exit
code when it quits.
...
Shell aliases (like `vim` → `nvim`) don't apply to spawned processes, so `vh`
resolves a real executable on `PATH` itself.
